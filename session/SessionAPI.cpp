...
void SessionAPI::receiveLoop() {
	...
	//printf("PollResult: %d\n", pollRes);
        ...
        //if() { // Sees if any data has been received in 1000ms. If not, just continue.
	//    printf("Broadcast!");
        //    broadcastStationAndState();
        //    continue;
        //}

	//printf("Trying to recieve from socketFD: %d\n", this->socketFD);
...
//        printf("RECEIVED %d BYTES\n\n", bytesReceived);
		...
		if(buffer[7] == this->sessionID) {
                    ...
                    this->replySessionIDInUse(&remoteAddr, buffer[6]);
                }
            ...
            case BEACON_SESSION:
                ...
                if(buffer[7] == this->sessionID) {
                    printf("[SessionAPI] Duplicate SessionID found!!!\n");
                    this->replySessionIDInUse(&remoteAddr, buffer[1]);
                }
                break;
            case DISCOVER:
...
void SessionAPI::replyStation(struct sockaddr_in* remoteAddress) {
    ...
    int res = sendto(this->socketFD, &finalArray, sizeof(finalArray), 0,(struct sockaddr *) &remoteAddress, sizeof(remoteAddress)); 
